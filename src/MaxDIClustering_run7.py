...
from osm_pk_info import get_pk_MiniBatchKMeans, get_pk_DBSCAN, get_initial_partition
...
def getRBFKernelsGraph(X, gamma=1., threshold=5., merage_components=False, n_jobs=-1, shpfile=False):
    ...
    if G.number_of_nodes() < X.shape[0]:
        ...
        for ndi in node_non_index:
            ...
            G = update_node_attributes(G, X, node_new=ndi)
            node_index.append(ndi)
        print("RBFKernelsGraph(Fixed): number_of_nodes:{0}, number_of_edges:{1}".format(G.number_of_nodes(), G.number_of_edges()))
    ...
    if merage_components:
        ...
        while Len_C >= 2:
            ...
            connected_components = [list(c) for c in sorted(nx.connected_components(G), key=len, reverse=False)]
            Len_C = len(connected_components)
        print("RBFKernelsGraph(Finally): number_of_nodes:{0}, number_of_edges:{1}".format(G.number_of_nodes(), G.number_of_edges()))
...
def main():
    ...
    outline_gdf = get_partion_result_outline(X=park_X, partion_result=partion_result, input_crs="EPSG:2436", output_crs=None,
                               shpfile="partion_outline_shp.shp", save_shp=True, plot=False)
    output = open('partion_result_outline_gdf.pkl', 'wb')
    pickle.dump(outline_gdf, output)
    output.close()
    print('------------------------------partion result outline end----------------------------------')
    ...
    # # test
    # print('X and Graph to shpfile')
    # getRBFKernelsGraph(X, gamma=1.0, threshold=0.998, merage_components=True,
    #                    n_jobs=-1, shpfile=True)
    # print('X and Graph to shpfile end')
